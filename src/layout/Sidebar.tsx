...
export function Sidebar({ activeTab, activeDownloadsCount, onTabChange }: SidebarProps) {
  ...
  const primaryTabs = sidebarTabs.filter((tab) => tab.id !== 'settings')
  const settingsTab = sidebarTabs.find((tab) => tab.id === 'settings')

  const renderTab = (tab: (typeof sidebarTabs)[number]) => (
    <button
      key={tab.id}
      className={activeTab === tab.id ? 'sidebar-link sidebar-link--active' : 'sidebar-link'}
      type="button"
      title={
        tab.id === 'downloads' && downloadsAria ? downloadsAria : t(tab.labelKey)
      }
      aria-label={
  ...
  return (
    <aside className="sidebar">
      ...
      {settingsTab ? (
        <nav className="sidebar__footer" aria-label={t('nav.settings')}>
